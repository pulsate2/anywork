...
- 顶部**终端全屏**与**键盘条显隐**开关:全屏把这一页抬到底部导航之上铺满视口(不是浏览器的网页全屏 —— 那会吞掉 Esc,且 iOS Safari 只给 `<video>`),同时把顶栏整条收起(改成浮层移出画外,不占布局,进出不触发终端重排),配合收掉键盘条能多出十几行。全屏状态记在地址栏(`?full=1`,进全屏时 push 一条历史),出去有两条路:**系统返回键**弹掉那条记录、或**长按键盘条上的 Esc**(500ms,Esc 本身照常在按下即发,长按只是追加一次退出)。同一路径只换 query,组件不重建、WebSocket 不重连。键盘条显隐记在 localStorage;全屏是临时姿势,不持久化。显隐开关只在触控设备出现(桌面本就没有这条)。
- 单指滚动滚回缓冲、双指/按钮缩放字体。
...
1. 壳自研:底部 Tab、工作区选择器、顶部状态徽标(当前 AI 档案),约 300 行。底部 Tab 是 `position: fixed` 贴 layout viewport 下沿,软键盘弹起时它会贴着键盘上沿冒出来挡住输入行(Chrome 只压缩 visual viewport 并把它往下挪,WebView 类浏览器把整页变矮,两种都会),所以检测到键盘就整条滑下去 —— 桌面的侧栏不适用这条。
