...
// 工具调用的详情内容(不带弹窗壳):编辑类 diff、ApplyPatch/CodexDiff、
// 截断信封兜底、结果、图片。从 ToolDetailModal 抽出,供两处复用 ——
// 全局详情弹窗(ToolDetailModal)与子 agent 过程弹窗(StepsModal)的右栏。
...
<!-- 弹窗内容 teleport 到 body,scoped 样式作用不到,放非 scoped 块。
     本组件在 ToolDetailModal(全局详情)与 StepsModal(子 agent 过程右栏)里
     都经 n-modal 渲染,规则跟着组件走,两处都能吃到。 -->
<style>
