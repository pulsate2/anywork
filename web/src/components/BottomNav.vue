...
import { TerminalOutline, FolderOpenOutline, FolderOutline, GitBranchOutline, SettingsOutline, SparklesOutline, ChatbubbleEllipsesOutline } from '@vicons/ionicons5'
...
// 沉浸模式(Agent 聊天这类全屏子视图打开时):同样滑下去。只在移动端生效 ——
// 桌面端这条导航是左侧栏,聊天是右边的列,收掉它就没有任何切页入口了。
...
const items = [
  ...
  { name: 'agent', label: 'Agent', path: '/agent', icon: ChatbubbleEllipsesOutline },
...
<template>
  ...
  <nav v-if="route.name !== 'login'" class="bottom-nav" :class="{ 'kb-open': kbOpen, immersive: immersive }">
    <router-link
      v-for="it in items"
      :key="it.name"
      :to="it.path"
