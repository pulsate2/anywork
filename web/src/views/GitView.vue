...
import {
  GitCompareOutline, GitCommitOutline, GitBranchOutline, CloudUploadOutline,
...
async function doCommit() {
  ...
  try {
    await api.gitCommit(repoPath.value, commitMsg.value, false)
    commitMsg.value = ''
...
<template>
  <div class="page-content">
    ...
    <n-modal v-model:show="commitModal" preset="card" title="提交" style="width: 92%; max-width: 560px">
      ...
      <div class="git-hint">只提交已暂存的改动({{ status?.staged.length || 0 }} 个文件)。</div>
      <template #footer>
...
.diff-body { overflow-x: auto; font-family: ui-monospace, monospace; font-size: 12px; line-height: 1.5; }
